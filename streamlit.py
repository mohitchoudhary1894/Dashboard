...
linechar = pd.DataFrame(filtered_df.groupby(filtered_df["month_year"].dt.strftime("%Y : %b"))["Sales"].sum()).reset_index()
fig2 = px.line(linechar, x = "month_year", y = "Sales", labels = {"Sales" : "Amount"}, height=500, width= 1000, template="gridon")
st.plotly_chart(fig2,use_container_width=True)

with st.expander("View Date of TimeSderies:"):
     st.write(linechar.T.style.background_gradient(cmap="Blues"))
     csv = linechar.to_csv(index=False).encode("utf-8")
...
with st.expander("Summary_Table"):
     ...
     st.markdown("Month wise sub - chategory table")
